...
class GithubCommitLogs:
	def __init__(self, parent):
		...
		self.APIs = [
			...
			("https://api.github.com/repos/BlackHole/enigma2/commits", "Enigma2"),  # expand later
		...
		self.skipCommits = ("openbh:", "openvix:", "PEP8 double aggressive")  # Stop showing changelog markers, these will be dropped in 6.0. Or PEP8 bot commits.
	...
	def readGithubCommitLogsSoftwareUpdateTask(self):
		...
		try:
			for c in self.fetchLog(url):
				...
				if self.getScreenTitle() != "Enigma2" and title.startswith(self.skipCommits) or title.startswith("openbh:") and not title.startswith(f"openbh: {SystemInfo['imagetype']} {SystemInfo['imageversion']}."):
					continue
