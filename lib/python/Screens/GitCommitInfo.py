...
def readGithubCommitLogsSoftwareUpdate():
	...
	try:
		...
		for c in log:
			if c['commit']['message'].startswith('openbh:') or (gitstart and not c['commit']['message'].startswith('openvix:') and getScreenTitle() in ("Enigma2", "OBH Core", "OBH Skins", "OE-A Core")):
					continue
