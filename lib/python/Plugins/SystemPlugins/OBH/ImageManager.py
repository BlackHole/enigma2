	...
	def keyRestore6(self, ret):
		...
		if SystemInfo["canMultiBoot"]:
			...
			if UBIMB:
				...
				print("[ImageManager] running commnd:%s slot = %s" % (CMD, self.multibootslot))
			elif CHKROOTMB:  # kernel:mmcblk0p1 root:mmcblk0p2 rootSubDir = None or mmcblk0p3
				CMD = "/usr/bin/ofgwrite -r%s -c%s -m%s '%s'" % (self.MTDROOTFS, SystemInfo["MultiBootSlot"], self.multibootslot, MAINDEST)
			else:
		...
		if wake - now < 60:
			...
			if not inStandby and config.imagemanager.query.value:
				if MODEL in ("dual"):
					...
					message = _("Your %s %s is about to create a full image backup, this can take between 10 to 15  minutes to complete.\nDo you want to allow this?") % (DISPLAYBRAND, MACHINENAME)
...
class ImageBackup(Screen):
	...
	def createBackupJob(self):
		...
		if SystemInfo["canBackupEMC"] and self.fullBackup:
			if MACHINEBUILD[0:7] != "osmio4kplus" or (MACHINEBUILD[0:7] == "osmio4kplus" and SystemInfo["MultiBootSlot"] == 1):
	...
	def doBackup3(self):
		...
		if self.EMMCIMG == "emmc.img":
			print("[ImageManager] osmio4kplus: EMMC Detected.")  # osmio4k+ receiver with multiple eMMC partitions in class
			blockSectors = 2
			...
			print(f"[ImageManager] osmio4kplus: EMMC commands:{self.commandMB}")  # osmio4k+ receiver with multiple eMMC partitions in class
	...
	def doBackup5(self):
		...
		if SystemInfo["brand"] == "vuplus":
			if MODEL == "vuzero":
				...
				with open(self.MAINDEST + "/reboot.update", "w") as fileout:
					...
					fileout.write(line)
		elif SystemInfo["brand"] in ("gigablue", "octagon", "edision"):
			if SystemInfo["brand"] in ("octagon", "edision"):
		...
		if path.exists(self.WORKDIR):
			rmtree(self.WORKDIR)
		if (path.exists(f"{self.MAINDEST}/{self.ROOTFSFILE}") and path.exists(f"{self.MAINDEST}/{self.KERNELFILE}")) or (MODEL in ("h9") and self.h9root):
