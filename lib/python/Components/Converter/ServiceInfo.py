...
class ServiceInfo(Poll, Converter):
	...
	def __init__(self, type):
		...
		self.type, self.interesting_events = {
			...
			"IsHLGFile2": (self.IS_HLG_FILE2, (iPlayableService.evVideoSizeChanged, iPlayableService.evUpdatedInfo, iPlayableService.evStart)),
		...
		if self.type in (self.HDR_TYPE, self.IS_HDR10_FILE, self.IS_HLG_FILE, self.IS_DV_FILE, self.IS_ANYHDR_FILE, self.IS_HDR_GENERIC_FILE, self.IS_HLG_FILE2):
	...
	def _readHLG(self):
		# 2=HLG for local files read from /tmp/hdrtype
		try:
			with open("/tmp/hdrtype", "r") as f:
				val = f.read().strip()
			return {"HLG": 2,}.get(val, 0)
		except Exception:
			return 0
	...
	def getBoolean(self):
		...
		if not info:
			return False
		elif self.type == self.IS_HLG_FILE2:
			return self._readHLG() == 2
		video_height = 0
		...
		if self.type == self.HAS_TELETEXT and not isRef:
			...
			if self.type == self.IS_WIDESCREEN:
			...
			elif self.type == self.IS_HLG_FILE2 and not isRef:
				...
				return self._readHDRType() > 0  # IsHDR (any HDR)
	...
	def getValue(self):
		...
		if self.type == self.XRES:
			...
			return info.getInfo(iServiceInformation.sHDRType)
		return -1
