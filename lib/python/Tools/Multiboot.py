	...
	for device in MbootList:
		...
		if path.exists(device):
			...
			if path.isfile(path.join(tmpname, "STARTUP")):  # Multiboot receiver
				if SystemInfo["HasKexecMultiboot"] and not path.isfile(path.join(tmpname, "etc/init.d/kexec-multiboot-recovery")) and path.isfile("/etc/init.d/kexec-multiboot-recovery"):  # check Recovery & slot image for recovery script
					if path.isfile(path.join(tmpname, "etc/issue")):
						...
						if Creator[0:6] in ("openvi", "openbh"):
							copyfile("/etc/init.d/kexec-multiboot-recovery", "%s" % path.join(tmpname, "etc/init.d/kexec-multiboot-recovery"))
